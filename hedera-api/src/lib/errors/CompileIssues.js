...
class CompileIssues extends Error {
    static tryThrowingIfErrorsIn({ compileResult = null, ignoreWarnings = true }) {
        if (null != compileResult && Array.isArray(compileResult.errors)) {
            const issuesReported = compileResult.errors.filter(err => (ignoreWarnings && err.type !== 'Warning') ||
                !ignoreWarnings);
...
exports.CompileIssues = CompileIssues;
//# sourceMappingURL=CompileIssues.js.map